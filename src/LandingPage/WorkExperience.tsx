...
function WorkExperience(props: any) {
    const { company, start_year, end_year, summary, logo_file, tech, labels } = props;
    const logo = new URL(`./assets/logos/${logo_file}`, import.meta.url).href
    ...
    return (
        <Grid size={12} container gap={5} className="whitebox">
            ...
            <Grid size="grow">
                <Stack spacing={2}>
                    <Typography variant='h6' component="h3">{company}</Typography>
                    <Typography>{start_year == end_year ? start_year : `${start_year} - ${end_year}`}</Typography>
